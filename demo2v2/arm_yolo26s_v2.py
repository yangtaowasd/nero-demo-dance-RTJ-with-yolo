...
class ArmYolo26sV2(Node):
    def __init__(self):
        ...
        self.declare_parameter("stable_motion_threshold_px", 180.0)
        self.declare_parameter("stable_range_threshold_px", 180.0)
        ...
        self.image_points_pub = self.create_publisher(Float32MultiArray, self.image_points_topic, 10)
    ...
    def log_camera_settings(self):
        ...
        self.get_logger().info(
            f"camera actual: id={self.camera_id}, fourcc={fourcc}, size={width}x{height}, fps={fps:.1f}"
        )
    ...
    def infer_loop(self):
        while self.running and rclpy.ok():
            ...
            try:
                ...
                image_points_msg = self.make_image_points_msg(output_records, frame.shape[:2], valid)
                with self.lock:
                    self.latest_msg = msg
    ...
    def update_pose_stability(self, records, width, height):
        ...
        if self.stable_pose_accepted:
            ...
            if elapsed < self.pose_read_duration:
                self.pose_status_text = f"reading pose {elapsed:.1f}/{self.pose_read_duration:.1f}s"
            else:
        ...
        if elapsed < self.stable_pose_duration:
            self.pose_status_text = f"capture stable {elapsed:.1f}/{self.stable_pose_duration:.1f}s"
            self.pose_status_ok = False
            return False
    ...
    def make_pose_msg(self, records, image_shape, valid):
        ...
        points = {name: self.image_to_camera_point(point, width, height) for name, point in records}
        data = [1.0]
