...
class AgxArmFollowerV2(Node):
    def __init__(self):
        ...
        self.camera_to_robot_matrix = read_matrix(self, "camera_to_robot_matrix")
    ...
    def build_target_xyz(self, shoulder, elbow, wrist, home_tcp):
        _, wrist_rel = self.get_relative_points(shoulder, elbow, wrist)
        robot_delta = self.motion_scale * (self.camera_to_robot_matrix @ wrist_rel)
    ...
    def arm_callback(self, side, msg):
        ...
        target_xyz = self.build_target_xyz(shoulder, elbow, wrist, home_tcp)
