...
class ArmIKNode(Node):
    def __init__(self):
        ...
        joint_status_out = self.get_parameter("joint_status_topic").value
        ...
        self.joint_status_sub = self.create_subscription(
            Float32MultiArray, joint_status_out, self.joint_status_callback, 10
        )
    ...
    def joint_status_callback(self, msg):
        if len(msg.data) != 14:
            self.get_logger().warning(f"Joint status data length error: {len(msg.data)}")
            return
        self.q_left = np.array(msg.data[0:7])
        self.q_right = np.array(msg.data[7:14])
